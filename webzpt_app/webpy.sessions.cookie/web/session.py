...
class Session(web.utils.Storage):
    ...
    def save(self):
        ...
        if not self._handler.sets_cookie:
...
class CookieHandler(Handler):
    ...
    def store(self, id_, client_ip, data, old_id=False):
        ...
        seed = randpool.RandomPool().get_bytes(
            web.config.handler_parameters.cookie_seed_length)

        box = pickle.dumps(web.Storage({
                'id': id_,
                'ip': client_ip,
                'data': data,
                'created': created,
        ...
        aes_seed = randpool.RandomPool().get_bytes(
            web.config.handler_parameters.cookie_seed_length)
        aes_obj = AES.new(aes_seed, AES.MODE_ECB)

        # loading RSA key object
        file_desc = file(web.config.handler_parameters.cookie_rsa_key_path, 'rb')
        rsa_obj = pickle.load(file_desc)
        file_desc.close()

        # encrypting key & data
        encrypted_key = pickle.dumps(rsa_obj.encrypt(aes_seed, ''))

        box += 'd' * (web.config.handler_parameters.cookie_seed_length -   \
            len(box) % web.config.handler_parameters.cookie_seed_length)

        encrypted_data = pickle.dumps(aes_obj.encrypt(box))

        # setting main data cookie and the "obscurity" cookie
        web.setcookie(web.config.session_parameters.cookie_name,
            encrypted_key,
            web.config.session_parameters.timeout,
            web.config.session_parameters.cookie_domain)

        web.setcookie(web.config.handler_parameters.cookie_aes_name,
            encrypted_data,
            web.config.session_parameters.timeout,
            web.config.session_parameters.cookie_domain)
    ...
    def retreive(self, id_):
        ...
        #return web.Storage()
        try:
            encrypted_key = web.cookies().__getattr__(web.config.session_parameters.cookie_name)
            encrypted_data = web.cookies().__getattr__(web.config.handler_parameters.cookie_aes_name)
        except (AttributeError, KeyError):
            return web.Storage()
        ...
        # getting AES seed
        aes_seed = rsa_obj.decrypt(pickle.loads(encrypted_key))
        aes_obj = AES.new(aes_seed, AES.MODE_ECB)

        # reviving data
        decrypted = aes_obj.decrypt(pickle.loads(encrypted_data))
        box = pickle.loads(decrypted.rstrip('d'))

        now = int(time.time())

        # checking validity
        try:
            # need test for valid id & data
            if not web.validipaddr(box.ip):
                raise AttributeError

            if now - box.touched > 24 * 3600 * 365  \
                or now - box.touched > 24 * 3600 * 365  \
                or box.touched - box.created < 0:
                raise AttributeError
...
def generate_key(file_name, key_size):
    '''generates RSA key of size, then pickles it into file'''
    rsa_key = RSA.generate(key_size, randpool.RandomPool().get_bytes)
    print rsa_key

    file_desc = file(file_name, 'wb')
    pickle.dump(rsa_key, file_desc, 0)
    file_desc.close()
